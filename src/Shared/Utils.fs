...
let ChangeCellStateBasedOnNeighbours (cells: Option<Cell> list) (cell: Cell ) : Cell=
    ...
    let amountOfAliveCellsAround = List.filter CellIsAlive cells |> List.filter  (fun cell -> fst cell.Value = Alive)
...
let CalculateTick (grid: CellGrid) : CellGrid =
    ...
    let aliveOnlyCells = List.filter  (fun cell -> fst cell = Alive) gridCells
